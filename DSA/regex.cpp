...
public:
    ...
    int GetData(int a) {
        std::cout << "From int GetData" << std::endl;
        return 0;
    };

    int GetData(float b) {
        std::cout << "From float GetData" << std::endl;
        return 0;
    }
};

int main() {
    QMS<int> *qns = new QMS<int>();
    qns->PutSampleData(0);
    std::cout << qns->GetSampleData(0.0);
    // delete qns; // Don't forget to free the allocated memory
    return 0;
}
